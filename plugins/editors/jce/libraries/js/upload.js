...
var Uploader = new Class({
        ...
        if(this.files.length) {
            ...
            if(index < this.files.length) {
                ...
                if(!this.current.complete) {
                    ...
                    this.current.element.adopt(new Element('input').setProperties({type: 'hidden', name: 'upload-name', value: this.fileinfo(this.current.element.getText()).name}));
                    this.iframe.submit();
                }
            } else {
                this.onAllComplete();
    ...
    fileCreate: function(file) {
        ...
        file.element = new Element('li').addClass('queue-text').addClass('file').addClass(f.ext.toLowerCase()).adopt([text, status, rename, file.input || '']).injectInside(this.list);
